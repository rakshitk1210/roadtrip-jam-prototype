import { emojiFor, type LngLat, type Place, type SavedPlace } from './places'
...
// Places bills per request rather than per result, so asking each anchor for its
// full allowance is what stocks the map's ambient tier for free.
const PER_ANCHOR = 16
...
/** How many the Discover sheet lists. */
const MAX_RESULTS = 12
/** How many more ride along on the map, uncovered by zooming in. */
const MAX_AMBIENT = 30

// v3 splits the results into the listed and ambient tiers; v4 carries `category`,
// so a v3 entry would render saved rows without it.
const CACHE_KEY = 'roadtrip-jam:discover:v4'
...
function toSavedPlace(p: ApiPlace, key: string, index: number): SavedPlace | null {
  ...
  return {
    ...
    category: kind,
    // Places has no emoji of its own, so the kind of place it says this is
    // picks one out of the design's vocabulary.
    emoji: emojiFor(`${kind ?? ''} ${name}`),
    knowBeforeYouGo: [
...
/**
 * Discover results in two tiers. The sheet lists the best of them; the rest ride
 * along on the map as the places zooming in uncovers. Both are full places, so
 * everything on the map opens the same detail sheet.
 */
 ...
 */
export async function discoverAlongRoute(): Promise<DiscoverResult | null> {
  const key = import.meta.env.VITE_GOOGLE_MAPS_KEY
  if (!key) return null

  try {
    const cached = sessionStorage.getItem(CACHE_KEY)
    if (cached) return JSON.parse(cached) as DiscoverResult
  } catch {
  ...
  try {
    ...
    const ranked = places.sort((a, b) => b.rating - a.rating || b.reviews - a.reviews)
    const result: DiscoverResult = {
      listed: ranked.slice(0, MAX_RESULTS),
      ambient: ranked.slice(MAX_RESULTS, MAX_RESULTS + MAX_AMBIENT),
    }
